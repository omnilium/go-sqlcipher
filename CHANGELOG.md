...
### Fixed
- Closed several connection-lifecycle leaks: `Open` now releases the SQLite handle (and its OS file lock) on every error path, virtual-table cursors and tables are freed as they close instead of accumulating until the connection closes, and `GetFilename`/`LoadExtension` no longer leak C memory.
- Panics raised inside a custom function, aggregator, or unlock-notify callback are now surfaced as SQL errors instead of unwinding across the cgo boundary and aborting the process; out-of-memory conditions in aggregates and `Deserialize` are likewise reported as errors rather than crashing.

### Security
- Encryption keys (and other driver DSN parameters) are no longer forwarded into SQLite's retained URI buffer when opening a `file:` DSN, where a loaded extension, custom VFS, or authorizer could read them back via `sqlite3_uri_parameter`. The driver's `_`-prefixed parameters are now stripped before open; SQLite's own URI parameters (`cache`, `mode`, `vfs`, `immutable`, …) are preserved unchanged.
...
[0.3.0]: https://github.com/omnilium/go-sqlcipher/compare/v0.2.0...v0.3.0
